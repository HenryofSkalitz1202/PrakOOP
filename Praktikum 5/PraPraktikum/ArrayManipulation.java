...
class ArrayManipulation {
    ...
    public static <T extends Comparable<T>> T[] mergeAndSortArrays(T[] arr1, T[] arr2) {
        T[] res_arr = Arrays.copyOf(arr1, arr1.length + arr2.length);
        System.arraycopy(arr2, 0, res_arr, arr1.length, arr2.length);
        Arrays.sort(res_arr);
        return res_arr;
    }
    ...
    public static <T> T[] mapArray(T[] arr, Function<T, T> mapper) {
        T[] res_arr = Arrays.copyOf(arr, arr.length);
        for(int i = 0; i < arr.length; i++){
            res_arr[i] = mapper.apply(arr[i]);
        }
    ...
    public static <T> T[] filterArray(T[] arr, Predicate<T> predicate) {
        ...
        T[] res_arr = Arrays.copyOf(arr, count);
        int idx = 0;
        for(T elmt : arr){
            if(predicate.test(elmt)){
                res_arr[idx] = elmt;
