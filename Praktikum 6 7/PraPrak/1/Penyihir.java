    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if(target.isHidup()){
            ...
            if(target.getMana() < 40){
                reducedMana = target.getMana();
            }

            System.out.println(self.getName() + " menyerap " + reducedMana + " mana dari " + target.getName());
            target.reduceMana(reducedMana);
            self.addMana(reducedMana);
