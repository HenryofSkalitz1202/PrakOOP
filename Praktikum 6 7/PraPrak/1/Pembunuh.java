    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if(target.isHidup()){
            ...
            if(target.getHealth() < 20){
                healthSerap = target.getHealth();
            }

            System.out.println(self.getName() + " menyerap health " + target.getName() + " sebesar " + healthSerap);
            target.terimaSerangan(healthSerap);
            self.addHealth(healthSerap);
        }else{
